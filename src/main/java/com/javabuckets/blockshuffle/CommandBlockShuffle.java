...
public class CommandBlockShuffle implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (args.length >= 2) {
                if (Collections.singletonList(Arrays.toString(ShuffleMode.values())).contains(args[0])) {
                    if (isNumeric(args[1])) {
                        if (args.length == 2) {
                        ...
                        BlockShuffle.initialize(contestants, ShuffleMode.valueOf(args[0]), Integer.valueOf(args[1]));
                        return true;
                    }
                }
            }
